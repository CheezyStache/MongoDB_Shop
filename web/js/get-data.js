function getItems() {
  fetch("https://localhost:5001/api/test/items").then((result) =>
    result.json().then((json) => {
      var itemCardsElement = document.getElementById("item-cards");
      if (itemCardsElement != undefined) itemCardsElement.innerHTML = "";
      json.forEach((element) => {
        document.getElementById(
          "items-list"
        ).innerHTML += `<li><a href="">${element.Name}</a></li>`;

        if (itemCardsElement != undefined)
...
function deleteItem(id) {
  fetch("https://localhost:5001/api/test/items?id=" + id, {
    method: "DELETE",
  }).then(() => {
    getItems();
  });
...
fetch("https://localhost:5001/api/test/customers").then((result) =>
  result.json().then((json) => {
    json.forEach((element) => {
      document.getElementById(
        "customers-list"
...
fetch("https://localhost:5001/api/test/orders").then((result) =>
  result.json().then((json) => {
    json.forEach((element) => {
      document.getElementById(
        "orders-list"
...
fetch("https://localhost:5001/api/test/carts").then((result) =>
  result.json().then((json) => {
    json.forEach((element) => {
      document.getElementById(
        "carts-list"
